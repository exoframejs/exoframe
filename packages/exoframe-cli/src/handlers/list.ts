...
export const listHandler = async () => {
  ...
  // get deployments
  try {
    const containers = await listDeployments({ endpoint, token: authToken });
    ...
    if (err instanceof Error && err.message === 'Authorization expired!') {
      await logout();
      console.log(chalk.red('Error: authorization expired!'), 'Please, relogin and try again.');
      return;
    }

    console.log(chalk.red('Error while getting list:'), err instanceof Error ? err.toString() : String(err));
  }
};
