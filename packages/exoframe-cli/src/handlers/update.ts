...
export const updateHandler = async (target?: 'server' | 'traefik' | 'all') => {
  if (!(await isLoggedIn())) {
  ...
  try {
    const updates = await checkUpdates({ endpoint, token: authToken });
    ...
    if (!target) {
      ...
      ({ upServer = false, upTraefik = false } = await inquirer.prompt<{ upServer?: boolean; upTraefik?: boolean }>(
        prompts
      ));
    }
    ...
    if (upServer && upTraefik) {
      ...
      await executeUpdate({ target: 'traefik', endpoint, token: authToken });
      await executeUpdate({ target: 'server', endpoint, token: authToken });
