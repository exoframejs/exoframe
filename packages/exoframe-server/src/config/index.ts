...
let loadedResolve: (value?: void | PromiseLike<void>) => void = () => {};
const isConfigLoaded = new Promise<void>((resolve) => {
...
const reloadUserConfig = () => {
  ...
  try {
    ...
    logger.debug(`loaded new config: ${JSON.stringify(userConfig)}`);
    loadedResolve();
  } catch (e) {
