...
import { createNetwork, initNetwork } from '../docker/network.ts';
 ...
 */
const portFromImage = async (image: string) => {
...
export async function startFromParams({
  ...
  let RestartPolicy: RestartPolicyConfig = {};
  const Name = ['no', 'on-failure', 'always'].find((c) => c.startsWith(restartPolicy)) ?? 'no';
  RestartPolicy = { Name };
  ...
  if (frontend?.length > 0) {
    ...
    Labels[`traefik.http.services.${projectName}.loadbalancer.server.port`] = String(usePort);
  ...
  const containerConfig: ContainerConfig = {
    ...
    Env: runtimeEnv,
    Labels,
    HostConfig: { RestartPolicy, Mounts },
  };
  ...
  // connect to additional networks when configured
  await Promise.all(
...
export async function start({ image, username, folder, resultStream }) {
  ...
  // create config
  const containerConfig: ContainerConfig = { Image: image, name, Env, Labels, HostConfig: { RestartPolicy } };

  // if volumes are set - add them to config
  if (config.volumes && config.volumes.length) {
    const mounts: MountConfig[] = config.volumes
      .map((vol) => vol.split(':'))
