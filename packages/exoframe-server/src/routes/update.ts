...
import { initNetwork } from '../docker/network.ts';
  ...
  fastify.route({
    ...
    async handler(request, reply) {
      ...
      if (target === 'server') {
        ...
        if (!oldServer?.Id) {
          reply.code(500).send({ updated: false, error: 'Existing server container not found', log: [] });
          return;
        }
        ...
        if (pullLog.includes('Downloaded newer image')) {
          ...
          const hash = serverImage?.Id?.split(':').pop()?.substring(0, 12) ?? 'latest';
          // init config
          const dockerConfig = {
