...
const exoServerUrl = `https://api.github.com/repos/exoframejs/exoframe/releases`;
...
const packageInfo = JSON.parse(packageString) as { version: string };
...
const getLatestVersion = async (url: string, versionPrefix?: string) => {
  const res = (await fetch(url).then((r) => r.json())) as ReleaseInfo[];
