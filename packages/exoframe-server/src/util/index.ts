...
import { getSecretsCollection } from '../db/secrets.ts';
...
export function runNPM({ args, cwd }) {
  return new Promise<RunLogEntry[]>((resolve) => {
    ...
    npm.on('exit', (code: number | null) => {
      log.push({ message: `npm exited with code ${String(code)}`, level: 'info' });
      resolve(log);
    });
  });
}
...
export function getEnv({ username, config, name, host, project = config.project || name }) {
  ...
  const secrets = getSecretsCollection().find({ user: username }) as SecretRecord[];
